...
export default function SocialPanel({ currentTrack, playlists, onPlayTrack, onPlayPlaylist, onImportPlaylist, notify }: { currentTrack: PipedTrack | null; playlists: Array<{ id: string; name: string; tracks: PipedTrack[] }>; onPlayTrack: (track: PipedTrack) => void; onPlayPlaylist: (tracks: PipedTrack[]) => void; onImportPlaylist: (name: string, tracks: PipedTrack[]) => void; notify: (message: string) => void }) {
  ...
  async function loadProfile(userId: string) {
    ...
    const { data, error } = await supabase.from("profiles").select("id,username,avatar_url,bio,country,last_seen,created_at,updated_at").eq("id", userId).single();
    if (error) throw error;
    setProfile(data as SocialProfile); return data as SocialProfile;
  }
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(async ({ data }) => {
      if (!alive) return;
      if (data.session) { setSessionReady(true); try { await loadProfile(data.session.user.id); } catch {} }
      else setSessionReady(false);
    });
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      setSessionReady(Boolean(session));
      if (session) void loadProfile(session.user.id).catch(()=>{}); else { setProfile(null); setConversations([]); setSelectedConversation(null); }
    });
  ...
  async function searchUsers(value: string) {
    ...
    const { data, error } = await supabase.from("profiles").select("id,username,avatar_url,bio,country,last_seen,created_at,updated_at").ilike("username", `%${query}%`).neq("id", profile.id).limit(20);
  ...
  if (!sessionReady) return <AuthPanel onReady={()=>setSessionReady(true)} />;
