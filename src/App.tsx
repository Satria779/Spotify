...
type CustomPlaylist = { id: string; name: string; tracks: PipedTrack[] };
...
export default function App() {
  ...
  function handleToggleLike(track:PipedTrack){setLikedTracks(prev=>({...prev,[track.id]:!prev[track.id]}));notify(likedTracks[track.id]?"Dihapus dari lagu yang disukai":"Ditambahkan ke lagu yang disukai");}
  ...
  function removeFromQueue(index:number){
    ...
    setPlayQueue(prev=>prev.filter((_,i)=>i!==index));setQueueIndex(i=>index<i?i:i);
  }
  ...
  function createPlaylist(){
    ...
    const playlist={id:`pl-${Date.now()}`,name:name.trim(),tracks:[]};setPlaylists(prev=>[...prev,playlist]);notify("Playlist dibuat.");
  }
  ...
  return <>
    ...
    <main className="app-shell">
      <section className={`view-section ${activeView==="home"?"active":""}`}>
        <div className="home-header no-avatar-header"><div className="pill active">Semua</div><div className="pill">Musik</div><div className="pill">Podcast</div></div>
      ...
      <section className={`view-section ${activeView==="library"?"active":""}`}>
        <div className="lib-header no-avatar-header"><div className="lib-header-left"><h1 className="lib-title">Koleksi Kamu</h1></div><button type="button" className="icon-btn" onClick={()=>setIsStatsOpen(true)} aria-label="Statistik"><StatsIcon/></button></div>
    ...
    {currentTrack&&!playerOpen?<button className="mini-player" onClick={()=>setPlayerOpen(true)}><img src={currentTrack.artwork} alt="Cover"/><div className="mini-player-info"><div className="mini-player-title">{currentTrack.title}</div><div className="mini-player-artist">{currentTrack.artist}</div></div><div className="mini-player-controls" onClick={e=>e.stopPropagation()}><button type="button" className="mini-icon-btn" onClick={()=>handleToggleLike(currentTrack)}><HeartIcon filled={likedTracks[currentTrack.id]}/></button><button type="button" className="mini-icon-btn" onClick={handleTogglePlay}>{isPlaying?<PauseIcon/>:<PlayIcon/>}</button></div></button>:null}
    ...
    <div className="hidden-player-host" aria-hidden="true"><ReactPlayer key={`${currentTrack?.id||"idle"}-${playbackSource?.mode||"none"}`} ref={playerRef} src={playbackSource?.src} playing={Boolean(currentTrack)&&isPlaying} controls={false} playsInline width={1} height={1} volume={volume/100} onPlay={()=>setIsPlaying(true)} onPause={()=>setIsPlaying(false)} onEnded={handleNext} onDurationChange={e=>setDuration(e.currentTarget.duration||currentTrack?.duration||0)} onTimeUpdate={e=>setCurrentTime(e.currentTarget.currentTime||0)} onError={()=>{setLoadingStream(false);notify("Audio gagal dimainkan.")}}/></div>
  </>;
}
