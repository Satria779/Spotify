...
export default function App() {
  ...
  const applyPlaybackRate=()=>{
    const media=playerRef.current||document.querySelector<HTMLMediaElement>(".hidden-player-host audio, .hidden-player-host video");
  ...
  useEffect(()=>{
    ...
    const apply=()=>{
      if(cancelled)return;
      if(applyPlaybackRate()||attempts>=12)return;
      attempts+=1;
