...
function AuthPanel({ onReady }: { onReady: () => void }) {
  ...
  async function resendConfirmation() {
    ...
    try {
      ...
      setNotice("Email konfirmasi dikirim ulang. Cek Inbox atau Spam.");
    } catch (err) { setError(errorText(err)); }
    finally { setBusy(false); }
  }

  async function resetPassword() {
    if (!supabase || !identifier.includes("@")) { setError("Masukkan email akun terlebih dahulu."); return; }
    setBusy(true); setError(""); setNotice("");
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(identifier.trim().toLowerCase(), { redirectTo: window.location.origin });
      if (error) throw error;
      setResetSent(true);
      setNotice("Link reset password dikirim ke email jika akun tersebut terdaftar.");
    } catch (err) { setError(errorText(err)); }
    finally { setBusy(false); }
  }
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      if (mode === "register") {
        ...
        const { data, error: signUpError } = await supabase.auth.signUp({ email: email.trim().toLowerCase(), password, options: { data: { username: username.trim().toLowerCase() }, emailRedirectTo: window.location.origin } });
        ...
        if (!value.includes("@")) {
          const { data, error: lookupError } = await supabase.functions.invoke("login-with-username", { body: { username: value.toLowerCase(), password } });
          if (lookupError || !data?.access_token || !data?.refresh_token) throw new Error("Username atau password salah.");
          ...
          if (loginError) throw new Error("Username/email atau password salah.");
        }
        onReady();
      }
    } catch (err) { setError(errorText(err)); }
  ...
  </form>{mode === "login" ? <div className="social-auth-extra"><button className="social-link-btn" type="button" onClick={()=>void resetPassword()} disabled={busy || resetSent}>Lupa password?</button>{notice.includes("konfirmasi") ? <button className="social-link-btn" type="button" onClick={()=>void resendConfirmation()} disabled={busy}>Kirim ulang email konfirmasi</button> : null}</div> : null}<button className="social-link-btn" type="button" onClick={()=>{setMode(mode === "login" ? "register" : "login");setError("");setNotice("");setResetSent(false)}}>{mode === "login" ? "Belum punya akun? Daftar" : "Sudah punya akun? Masuk"}</button></div></div>;
}
...
export default function SocialPanel({ currentTrack, playlists, onPlayTrack, onPlayPlaylist, onImportPlaylist, notify }: { currentTrack: PipedTrack | null; playlists: Array<{ id: string; name: string; tracks: PipedTrack[] }>; onPlayTrack: (track: PipedTrack) => void; onPlayPlaylist: (tracks: PipedTrack[]) => void; onImportPlaylist: (name: string, tracks: PipedTrack[]) => void; notify: (message: string) => void }) {
  ...
  const [socialView, setSocialView] = useState<"messages" | "search" | "profile" | "user">("messages");
  ...
  async function loadProfile(userId: string) {
    ...
    const columns = "id,username,avatar_url,bio,country,last_seen,created_at,updated_at";
  ...
  useEffect(() => {
    ...
    }).on("postgres_changes", { event: "UPDATE", schema: "public", table: "messages", filter: `receiver_id=eq.${profile.id}` }, payload => setMessages(prev=>prev.map(m=>m.id===payload.new.id?payload.new as SocialMessage:m)));
  ...
  async function searchUsers(value: string) {
    ...
    const { data, error } = await supabase.from("profiles").select("id,username,avatar_url,bio,country,last_seen,created_at,updated_at").ilike("username", `%${query}%`).neq("id", profile.id).order("username", { ascending: true }).limit(20);
  ...
  async function openUser(user: SocialProfile) {
    ...
    setFollowers(followerCount || 0); setFollowing(followingCount || 0); setFollowingUser(Boolean(follow)); setBlockedUser(Boolean(block));
  }
  ...
  async function saveProfile(e: FormEvent) {
    ...
    try {
      ...
      const { data, error } = await supabase.from("profiles").update({ username: editUsername.toLowerCase(), bio: editBio.trim().slice(0, 160), country: editCountry || null, avatar_url: avatarUrl }).eq("id", profile.id).select().single();
      if (error) throw error;
      if (newPassword) {
  ...
  async function signOut() { if (!supabase) return; await supabase.auth.signOut(); setSessionReady(false); setProfile(null); setProfiles([]); setConversations([]); setSelectedConversation(null); setMessages([]); }
